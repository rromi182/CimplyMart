...
if (isset($_GET['id_presupuesto'])) {
    ...
    $sqlProveedor = mysqli_query($mysqli, "SELECT cod_proveedor, razon_social, ruc FROM v_presupuesto_compra_orden WHERE id_presupuesto_compra = $idPresupuesto");
    ...
    $sqlProductos = mysqli_query($mysqli, "SELECT * FROM v_presupuesto_compra_orden WHERE id_presupuesto_compra = $idPresupuesto");
    ...
    while ($row = mysqli_fetch_array($sqlProductos)) {
        ...
        $data[] = [
            'idPresupuestoCompra' => $idPresupuestoCompra,
...
<div class="form-group row">
    <label class="col-sm-2 col-form-label" style="margin-left: 30px;">Proveedor:</label>
